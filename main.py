...
def checkout():
    ...
    if authenticated():
        if request.method == "GET":
            ...
            try:
                if 'cart_item' and 'all_total_quantity' and 'all_total_price' in session:
                    order_data = {"name": name, "address": address, "phone": phone, "email": session['email'], "created_at": created_at, "items": session[
...
def api_login():
    if request.method == "POST":  # Only if data has been posted
        ...
        try:
            ...
            # If there is any error, redirect back to login
            return Response(json.dumps({"error": "Wrong username/password"}), status=400, mimetype='application/json')
        session['email'] = email
        return Response(json.dumps({"success": "Successful authentication"}), status=200, mimetype='application/json')
    else:
        return Response(json.dumps({"error": "Method not POST"}), status=403, mimetype='application/json')
...
def api_register():
    if request.method == "POST":  # Only if data has been posted
        ...
        try:
            ...
            user = auth.sign_in_with_email_and_password(email, password)
            session['email'] = email
            return Response(json.dumps({"success": "Successful registration"}), status=200, mimetype='application/json')
        except:
            # If there is any error, display
            return Response(json.dumps({"error": "Error in registration"}), status=400, mimetype='application/json')
    else:
        return Response(json.dumps({"error": "Method not POST"}), status=403, mimetype='application/json')
...
def api_logout():
    if request.method == "GET":  
        # remove the email from the session if it is there
        session.pop('email', None)
        return Response(json.dumps({"success": "Successfully logged out"}), status=200, mimetype='application/json')
    else:
        return Response(json.dumps({"error": "Method not GET"}), status=403, mimetype='application/json')
            ...
            if authenticated():
                try:
                    ...
                    for p in products.each():
                        output.append({"id": p.key(), "name": p.val()['name'], "price": p.val()['price'], "sku": p.val()[
                            'sku'], "image": p.val()['image'], "created_at": p.val()['created_at']})
                    return Response(json.dumps({"success": output}), status=200, mimetype='application/json')
                except:
                    return Response(json.dumps({"error": "No products found"}), status=200, mimetype='application/json')
            else:  
                return Response(json.dumps({"error": "User not authenticated"}), status=403, mimetype='application/json')         
    except Exception as e:
        return Response(json.dumps({"error": e}), status=400, mimetype='application/json')
...
def api_product(id):
    if authenticated(): 
        try:
            products = client.collections['products'].documents[id].retrieve()
            try:
                return Response(json.dumps({"success": products}), status=200, mimetype='application/json')
            except Exception as e:
                return Response(json.dumps({"error": e}), status=400, mimetype='application/json')
        except Exception as e:
            return Response(json.dumps({"error": "Product not found"}), status=400, mimetype='application/json')
    ...
    if authenticated(): 
        ...
        try:
            ...
            try:
                ...
                order_data = {"email": session['email'], "items": total_items, "all_total_quantity": all_total_quantity, "all_total_price": all_total_price}
                return Response(json.dumps({"success": order_data}), status=200, mimetype='application/json')               
            except Exception as e:
                return Response(json.dumps({"error": e}), status=400, mimetype='application/json')               
        except Exception as e:
            return Response(json.dumps({"error": e}), status=400, mimetype='application/json')
...
def api_empty_cart():
    if authenticated():
        try:
            ...
            session.clear()
            session['email'] = email
            return Response(json.dumps({"success": "Successfully emptied cart"}), status=200, mimetype='application/json')
        except Exception as e:
            return Response(json.dumps({"error": e}), status=400, mimetype='application/json')
    else:
        return Response(json.dumps({"error": "User not authenticated"}), status=403, mimetype='application/json') 
            ...
            try:
                if 'cart_item' and 'all_total_quantity' and 'all_total_price' in session:
                    ...
                    try:
                        ...
                        session.clear()
                        session['email'] = email
                        return Response(json.dumps({"success": rec['name']}), status=200, mimetype='application/json')
                    except Exception as e:
                        return Response(json.dumps({"error": e}), status=400, mimetype='application/json')
                else:
                    return Response(json.dumps({"error": "No items in cart"}), status=400, mimetype='application/json')
            except Exception as e:
                return Response(json.dumps({"error": e}), status=400, mimetype='application/json')
    else:
        return Response(json.dumps({"error": "User not authenticated"}), status=403, mimetype='application/json') 
